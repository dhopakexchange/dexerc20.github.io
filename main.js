...
function dexerc20() {
  ...
  this.contractdexerc20 = undefined;
...
dexerc20.prototype.ejs = function ejs(url, element, data) {
  if ($(`#${element}`).length) {
...
dexerc20.prototype.dialogInfo = function dialogInfo(message) {
  console.log(message);
...
dexerc20.prototype.dialogError = function dialogError(message) {
  console.log(message);
...
dexerc20.prototype.alertSuccess = function alertSuccess(message) {
  console.log(message);
...
dexerc20.prototype.txError = function txError(err) {
  ...
  utility.getBalance(this.web3, this.addrs[this.selectedAccount], (errBalance, resultBalance) => {
    ...
    if (this.connection.connection === 'RPC' && this.addrKinds[this.selectedAccount] !== 'MetaMask') {
      if (this.pks[this.selectedAccount]) {
        this.dialogError('You are using an dexerc20 account that has a private key attached, but you\'re connected to MetaMask. You should disable MetaMask from Chrome\'s Window -> Extensions menu (don\'t worry, this won\'t lose your MetaMask data), then refresh dexerc20.');
        ga('send', {
          hitType: 'event',
          eventCategory: 'Error',
      ...
      this.dialogError('You are using an dexerc20 account that doesn\'t have a private key attached. Perhaps you created the account using MetaMask, in which case you should make sure MetaMask is enabled and logged in to this account, then refresh dexerc20. Or, if you have the private key, you can choose "Import account" from the accounts dropdown (upper right) to re-import the account with its private key.');
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
      ...
      this.dialogError('You are using an dexerc20 account that has an invalid private key.');
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
      ...
      this.dialogError(
        `Your wallet's ETH balance (${balance} ETH) is not enough to cover the gas cost (Ethereum network fee). dexerc20 sends ${this.minGas} ETH with each transaction. This is an overestimate and the excess will get refunded to you. It's a good idea to send more than ${this.minGas} so you can pay for not only this transaction, but also future transactions you do on dexerc20. The gas has to come directly from your Wallet (dexerc20 has no physical way of paying gas from your deposited ETH).`);
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
...
dexerc20.prototype.alertTxResult = function alertTxResult(err, txsIn) {
...
dexerc20.prototype.enableTooltipsAndPopovers = function enableTooltipsAndPopovers() {
  $('[data-toggle="popover"]').popover({ trigger: 'hover' });
  $('[data-toggle="tooltip"]').tooltip();
};
dexerc20.prototype.createAccount = function createAccount() {
...
dexerc20.prototype.deleteAccount = function deleteAccount() {
  if (this.pks[this.selectedAccount]) {
...
dexerc20.prototype.selectAccount = function selectAccount(i) {
...
dexerc20.prototype.addAccount = function addAccount(newAddr, newPk) {
...
dexerc20.prototype.showPrivateKey = function showPrivateKey() {
  const addr = this.addrs[this.selectedAccount];
  const pk = this.pks[this.selectedAccount];
  if (!pk) {
    this.dialogError(
...
dexerc20.prototype.addressLink = function addressLink(address) {
  return `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${address}`;
};
dexerc20.prototype.contractAddr = function contractAddr(addr) {
  this.config.contractdexerc20Addr = addr;
  this.displayConnectionDescription(() => {});
  this.loading(() => {});
  this.refresh(() => {}, true, true);
};
dexerc20.prototype.displayAccounts = function displayAccounts(callback) {
...
dexerc20.prototype.displayLanguages = function displayLanguages(callback) {
...
dexerc20.prototype.selectLanguage = function selectLanguage(newLanguage) {
...
dexerc20.prototype.loadEvents = function loadEvents(callback) {
  let lastBlock = 0;
  Object.keys(this.eventsCache).forEach((id) => {
    const event = this.eventsCache[id];
    if (event.blockNumber > lastBlock && event.address === this.config.contractdexerc20Addr) {
...
dexerc20.prototype.displayMyTransactions =
function displayMyTransactions(ordersIn, blockNumber, callback) {
  ...
  orders = orders.filter(order => order.order.contractAddr === this.config.contractdexerc20Addr);
  ...
  events.forEach((event) => {
    try {
      if (
        event.event === 'Trade' &&
        event.address === this.config.contractdexerc20Addr &&
...
dexerc20.prototype.isInCross = function isInCross(priceIn, kind) {
  ...
  .filter(
    order => order.order.contractAddr === this.config.contractdexerc20Addr);
  // final order filtering and sorting
...
dexerc20.prototype.displayVolumes = function displayVolumes(
...
dexerc20.prototype.displayTradesAndChart = function displayTradesAndChart(callback) {
  // get the trade list
  const events = Object.values(this.eventsCache);
  const trades = [];
  events.forEach((event) => {
    if (event.event === 'Trade' && event.address === this.config.contractdexerc20Addr) {
      if (event.args.amountGive.toNumber() > 0 && event.args.amountGet.toNumber() > 0) {
        // don't show trades involving 0 amounts
        let trade;
        if (
...
dexerc20.prototype.candlestickChart =
...
dexerc20.prototype.getOrdersByPair = function getOrdersByPair(tokenA, tokenB, callback) {
...
dexerc20.prototype.getReturnTicker = function getTopOrders(callback) {
...
dexerc20.prototype.getTopOrders = function getTopOrders(callback) {
...
dexerc20.prototype.displayOrderbook = function displayOrderbook(ordersIn, blockNumber, callback) {
  // only look at orders for the selected token and base
  let orders = ordersIn.filter(
    x =>
      (x.order.tokenGet.toLowerCase() === this.selectedToken.addr.toLowerCase() &&
...
dexerc20.prototype.displayTokensAndBases = function displayTokensAndBases(callback) {
...
dexerc20.prototype.displayAllBalances = function displayAllBalances(callback) {
  ...
  async.map(
    ...
    (token, callbackMap) => {
      if (token.addr === zeroAddr) {
        utility.call(
          this.web3,
          this.contractdexerc20,
          this.config.contractdexerc20Addr,
          'balanceOf',
...
dexerc20.prototype.transfer = function transfer(addr, inputAmount, toAddr) {
  ...
  } else if (toAddr.toLowerCase() === this.config.contractdexerc20Addr.toLowerCase()) {
    this.dialogError('If you send funds directly to the dexerc20 smart contract, they will be lost. You need to use the Deposit tab to deposit.');
    ga('send', {
      hitType: 'event',
      eventCategory: 'Error',
      eventAction: 'Transfer - invalid address',
...
dexerc20.prototype.deposit = function deposit(addr, inputAmount) {
  let amount = new BigNumber(Number(utility.ethToWei(inputAmount, this.getDivisor(addr))));
  const token = this.getToken(addr);
  if (amount.lte(0)) {
  ...
  if (addr.slice(0, 39) === '0x0000000000000000000000000000000000000') {
    ...
    utility.call(
      ...
      token.addr,
      'allowance',
      [this.addrs[this.selectedAccount], this.config.contractdexerc20Addr],
                  ...
                  (callbackSeries) => {
                    if (resultAllowance.eq(0)) {
                      utility.send(
                        ...
                        addr,
                        'approve',
                        [this.config.contractdexerc20Addr, amount,
...
dexerc20.prototype.withdraw = function withdraw(addr, amountIn) {
...
dexerc20.prototype.order = function order(direction, amount, price, expires, refresh) {
...
dexerc20.prototype.publishOrder = function publishOrder(
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        hash, this.pks[this.selectedAccount], (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.config.contractdexerc20Addr,
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
...
dexerc20.prototype.cancelOrder = function cancelOrder(orderIn) {
...
dexerc20.prototype.trade = function trade(kind, order, inputAmount) {
  if (this.addrs[this.selectedAccount].slice(0, 39) === '0x0000000000000000000000000000000000000') {
    this.dialogError(
      "You haven't selected an account. Make sure you have an account selected from the Accounts dropdown in the upper right.");
    ga('send', {
...
dexerc20.prototype.addPending = function addPending(err, txsIn) {
  const txs = Array.isArray(txsIn) ? txsIn : [txsIn];
  txs.forEach((tx) => {
...
dexerc20.prototype.updateUrl = function updateUrl() {
...
dexerc20.prototype.getDivisor = function getDivisor(tokenOrAddress) {
...
dexerc20.prototype.getToken = function getToken(addrOrToken, name, decimals) {
  let result;
...
dexerc20.prototype.loadToken = function loadToken(addr, callback) {
...
dexerc20.prototype.selectToken = function selectToken(addrOrToken, name, decimals) {
...
dexerc20.prototype.selectBase = function selectBase(addrOrToken, name, decimals) {
...
dexerc20.prototype.selectTokenAndBase = function selectTokenAndBase(tokenAddr, baseAddr) {
...
dexerc20.prototype.setGasPrice = function setGasPrice(gasPrice) {
  if (gasPrice) {
...
dexerc20.prototype.displayBuySell = function displayBuySell(callback) {
...
dexerc20.prototype.displayTokenGuidesDropdown = function displayTokenGuidesDropdown() {
  const tokens = this.config.tokens.map(x => x);
  tokens.sort((a, b) => (a.name > b.name ? 1 : -1));
...
dexerc20.prototype.displayHelpDropdown = function displayHelpDropdown() {
  this.ejs(`${this.config.homeURL}/templates/helpDropdown.ejs`, 'helpDropdown', {});
};
dexerc20.prototype.displayHelp = function displayHelp(name) {
  $('#helpBody').html('');
...
dexerc20.prototype.displayScreencast = function displayScreencast(name) {
  $('#screencastBody').html('');
...
dexerc20.prototype.displayConnectionDescription = function displayConnectionDescription() {
  this.ejs(`${this.config.homeURL}/templates/connectionDescription.ejs`, 'connection', {
    connection: this.connection,
    contracts: this.config.contractdexerc20Addrs,
    contractAddr: this.config.contractdexerc20Addr,
    contractLink: `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${this.config.contractdexerc20Addr}`,
  });
};
dexerc20.prototype.displayTokenGuide = function displayTokenGuide(name) {
...
dexerc20.prototype.checkContractUpgrade = function checkContractUpgrade() {
  if (
    (!this.selectedContract || this.selectedContract !== this.config.contractdexerc20Addr) &&
    (this.addrs.length > 1 ||
      (this.addrs.length === 1 && this.addrs[0].slice(0, 39) !== '0x0000000000000000000000000000000000000'))
  ) {
    this.dialogInfo(
      '<p>dexerc20 has a new smart contract. It is now selected.</p><p>Please use the "Smart Contract" menu to select the old one and withdraw from it.</p><p><a href="javascript:;" class="btn btn-default" onclick="alertify.closeAll(); bundle.dexerc20.displayHelp(\'smartContract\')">Smart contract changelog</a></p>');
  }
};
dexerc20.prototype.resetCaches = function resetCaches() {
...
dexerc20.prototype.loading = function loading(callback) {
  [
    'deposit',
    'withdraw',
    'buy',
...
dexerc20.prototype.refresh = function refresh(callback, forceEventRead, initMarket, token, base) {
  this.q.push((done) => {
    if (token && base) {
    ...
    this.selectedContract = this.config.contractdexerc20Addr;
...
dexerc20.prototype.refreshLoop = function refreshLoop() {
  const self = this;
  function loop() {
    self.refresh(() => {
...
dexerc20.prototype.initDisplays = function initDisplays(callback) {
  this.loading(() => {});
...
dexerc20.prototype.loadWeb3 = function loadWeb3(callback) {
  this.config = config;
  // web3
...
dexerc20.prototype.initContracts = function initContracts(callback) {
  this.web3.version.getNetwork((error, version) => {
    ...
    // connection
    this.config.contractdexerc20Addr = this.config.contractdexerc20Addrs[0].addr;
    // get accounts
    ...
    utility.loadContract(
      this.web3,
      this.config.contractdexerc20,
      this.config.contractdexerc20Addr,
      (err, contractdexerc20) => {
        this.contractdexerc20 = contractdexerc20;
        utility.loadContract(
          this.web3,
...
dexerc20.prototype.startdexerc20 = function startdexerc20() {
...
const dexerc20 = new dexerc20();

module.exports = { dexerc20: dexerc20, dexerc20 };
