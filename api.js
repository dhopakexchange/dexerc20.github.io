...
API.init = function init(callback, allContracts, path, provider, configName, lookbackIn) {
  ...
  this.web3.version.getNetwork((error, version) => {
    ...
    // path
    if (path) {
      this.config.contractdexerc20 = path + this.config.contractdexerc20;
      this.config.contractToken = path + this.config.contractToken;
    }

    // contracts
    this.contractdexerc20 = undefined;
    this.contractdexerc20Addrs = [this.config.contractdexerc20Addrs[0].addr];
    if (allContracts) {
      this.contractdexerc20Addrs = this.config.contractdexerc20Addrs.map(x => x.addr);
    }
    ...
    async.series(
      [
        (callbackSeries) => {
          this.utility.loadContract(
            ...
            this.contractdexerc20Addrs[0],
            (err, contract) => {
              this.contractdexerc20 = contract;
      ...
      () => {
        callback(null, {
          contractdexerc20: this.contractdexerc20,
...
API.logs = function logs(callback, lookbackIn) {
  this.utility.blockNumber(this.web3, (err, blockNumber) => {
    ...
    async.mapSeries(
      this.contractdexerc20Addrs,
      (contractdexerc20Addr, callbackMap) => {
        const blocks = Object.values(this.eventsCache)
          .filter(x => x.address === contractdexerc20Addr)
          .map(x => x.blockNumber);
...
API.getdexerc20Balance = function getdexerc20Balance(addr, callback) {
  if (addr.length === 42) {
...
API.getdexerc20TokenBalances = function getdexerc20TokenBalances(addr, callback) {
  if (addr.length === 42) {
    async.reduce(
      this.config.tokens,
      {},
...
API.getUSDBalance = function getUSDBalance(addr, tokenPrices, callback) {
  async.parallel(
    [
      ...
      (callbackParallel) => {
        API.getdexerc20TokenBalances(addr, callbackParallel);
      },
      (callbackParallel) => {
    ...
    (err, results) => {
      const balances = { Wallet: results[0], dexerc20: results[1] };
...
API.addOrderFromEvent = function addOrderFromEvent(event, callback) {
  if (event.event === 'Order' && event.address === this.contractdexerc20Addrs[0]) {
...
API.getTrades = function getTrades(callback) {
  ...
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractdexerc20Addrs.indexOf(event.address) >= 0) {
...
API.getVolumes = function getVolumes(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractdexerc20ddrs.indexOf(event.address) >= 0) {
      if (event.args.amountGive.toNumber() > 0 && event.args.amountGet.toNumber() > 0) {
        // don't show trades involving 0 amounts
        volumes.push({
          token: API.getToken(event.args.tokenGive),
...
API.getDepositsWithdrawals = function getDepositsWithdrawals(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Deposit' && this.contractdexerc20Addrs.indexOf(event.address >= 0)) {
      ...
      event.event === 'Withdraw' && this.contractdexerc20Addrs.indexOf(event.address) >= 0
    ) {
      if (event.args.amount.toNumber() > 0) {
        const token = API.getToken(event.args.token);
        depositsWithdrawals.push({
      ...
      if (trade.token && trade.base && trade.base.name === 'ETH') {
        ...
        if (!tickers[pair]) {
          ...
          ordersFiltered = ordersFiltered.filter(
            order => order.order.contractAddr === this.config.contractdexerc20Addrs[0].addr);
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        this.utility.sign(this.web3, addr, hash, pk, (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.contractdexerc20Addrs[0],
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
