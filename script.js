...
$(() => {
  $('body').on('click', '#accountSubmit', (e) => {
    ...
    bundle.dexerc20.addAccount($('#accountAddr').val(), $('#accountPk').val());
  });
});
function buyChange() { // eslint-disable-line no-unused-vars
  ...
  const isInCross = bundle.dexerc20.isInCross(price, 'buy');
  if (isInCross) {
...
function sellChange() { // eslint-disable-line no-unused-vars
  ...
  const total = (amount * price).toFixed(3);
  const isInCross = bundle.dexerc20.isInCross(price, 'sell');
  if (isInCross) {
...
$(() => {
  $('body').on('click', '#buyCrossSubmit', (e) => {
    ...
    bundle.dexerc20.trade('buy', JSON.parse($('#buyCrossOrder').val()), $('#buyCrossAmount').val());
  });
});
$(() => {
...
$('#gasPriceModal').on('show.bs.modal', () => {
  $('#gasPrice').val(bundle.dexerc20.config.ethGasPrice / 1000000000);
});
$(() => {
  $('body').on('click', '#gasPriceSubmit', (e) => {
    e.preventDefault();
    $('#gasPriceModal').modal('hide');
    bundle.dexerc20.setGasPrice(
      $('#gasPrice').val());
  });
});
function depositClick(addr) { // eslint-disable-line no-unused-vars
  bundle.dexerc20.deposit(addr, $(`#depositAmount${addr}`).val());
}
function withdrawClick(addr) { // eslint-disable-line no-unused-vars
  bundle.dexerc20.withdraw(addr, $(`#withdrawAmount${addr}`).val());
}
function transferClick(addr) { // eslint-disable-line no-unused-vars
  bundle.dexerc20.transfer(addr, $(`#transferAmount${addr}`).val(), $(`#transferTo${addr}`).val());
}
$(() => {});
