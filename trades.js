...
const addressdexerc20 = '0x051a4A698b3fA71c2e84B9f1ce5977EA6dD0799e';
const addressToken = '0x051a4A698b3fA71c2e84B9f1ce5977EA6dD0799e';
...
function TradeUtil() {
  ...
  self.getContract = function getContract(callback) {
    self.get(`https://api.etherscan.io/api?module=contract&action=getabi&address=${addressdexerc20}`, (err, data) => {
      if (err) throw new Error(err);
      const abi = JSON.parse(data.result);
      self.contractdexerc20 = web3.eth.contract(abi);
      self.contractdexerc20 = self.contractdexerc20.at(addressdexerc20);
      callback(null, self.contractdexerc20);
    });
  };
  ...
  self.getLog = function getLog(fromBlock, toBlock, callback) {
    function decodeEvent(item) {
      const eventAbis = self.contractdexerc20.abi.filter(eventAbi => (
          eventAbi.type === 'event' &&
          item.topics[0] ===
            `0x${
              sha3(
      ...
      if (eventAbis.length > 0) {
        ...
        const event = new SolidityEvent(web3, eventAbi, addressdexerc20);
    ...
    const url =
      `https://api.etherscan.io/api?module=logs&action=getLogs&address=${addressdexerc20}&fromBlock=${fromBlock}&toBlock=${toBlock}`;
